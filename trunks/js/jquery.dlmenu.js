...
;( function( $, window, undefined ) {
	...
	$.DLMenu.defaults = {
		...
		animationClasses : { classin : 'mobile-animate-in-1', classout : 'mobile-animate-out-1' },
	...
	$.DLMenu.prototype = {
		...
		_config : function() {
			this.open = false;
			this.$trigger = this.$el.children( '.mobile-trigger' );
			this.$menu = this.$el.children( 'ul.mobile-menu' );
			this.$menuitems = this.$menu.find( 'li:not(.mobile-back)' );
			this.$el.find( 'ul.mobile-submenu' ).prepend( '<li class="mobile-back"><a href="#"><i class="fa fa-angle-left"></i> </a></li>' );
		...
		_initEvents : function() {
			...
			this.$menuitems.on( 'click.dlmenu', function( event ) {
				...
				var $item = $(this),
					$submenu = $item.children( 'ul.mobile-submenu' );
			...
			this.$back.on( 'click.dlmenu', function( event ) {
				...
				var $this = $( this ),
					$submenu = $this.parents( 'ul.mobile-submenu:first' ),
					$item = $submenu.parent(),
		...
		_closeMenu : function() {
			...
			this.$menu.removeClass( 'mobile-menuopen' );
			this.$menu.addClass( 'mobile-menu-toggle' );
			this.$trigger.removeClass( 'mobile-active' );
		...
		_openMenu : function() {
			...
			this.$menu.addClass( 'mobile-menuopen mobile-menu-toggle' ).on( this.transEndEventName, function() {
				$( this ).removeClass( 'mobile-menu-toggle' );
			} );
			this.$trigger.addClass( 'mobile-active' );
			this.open = true;
		},
